...
def calculate_retrieval_scores(results):
    ...
    for _, scores in retrieval_scores.items():
        scores["ave_recall"] = round(float(scores["total_recall"] / scores["count"]), 4)
        scores["ave_ndcg"] = round(float(scores["total_ndcg"] / scores["count"]), 4)
...
def calculate_qa_scores(results, outdated_values_dict):
    ...
    for category_name, scores in end2end_scores.items():
        if "outdated_information" in category_name.lower():
            ...
            scores["ave_precision"] = round(float(scores["total_precision"] / scores["count"]), 4)
            scores["ave_recall"] = round(float(scores["total_recall"] / scores["count"]), 4)
            scores["ave_f1"] = round(float(scores["total_f1"] / scores["count"]), 4)
...
def eval(args):
    ...
    for result_dir in result_dirs:
        ...
        data = table_all_noise
        rows = []
        for eval_type, systems in data[0]["Statistics"].items():
            for system_name, categories in systems.items():
                for category_name, metrics in categories.items():
                    if category_name=="outdated_information":
                        continue
                    row = {
                        "eval_type": eval_type,
